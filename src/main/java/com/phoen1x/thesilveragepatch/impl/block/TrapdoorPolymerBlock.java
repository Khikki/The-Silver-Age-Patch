...
public record TrapdoorPolymerBlock() implements FactoryBlock, PolymerTexturedBlock, BSMMParticleBlock {
    ...
    private static final Map<Direction, BlockState> STATES_REGULAR = Arrays.stream(Direction.values())
            .collect(Collectors.toMap(x -> x, x -> PolymerBlockResourceUtils.requestEmpty(BlockModelType.getTrapdoor(x, false))));
    private static final Map<Direction, BlockState> STATES_WATERLOGGED = Arrays.stream(Direction.values())
            .collect(Collectors.toMap(x -> x, x -> PolymerBlockResourceUtils.requestEmpty(BlockModelType.getTrapdoor(x, true))));
